...
namespace MyFaceDNN
{
    public class FaceProccesOpenCl
    {
        
        private Net faceDetector,embedder = null;
        private FacemarkLBF facemark = null;
        EigenFaceRecognizer recognizer;
        public FaceProccesOpenCl()
        {
            if (facemark == null)
            {
        ...
        public void DetectAndRender(UMat image, DepthFrame depthFrame = null)
        {
            ...
            if (fullFaceRegions.Count > 0)
            {
                ...
                {
                    //var faceVector = GetFeatureVector(new UMat(image, face));
        ...
        private Mat GetFeatureVector(UMat face)
        {
            int imgDim = 96;
            ...
            inputBlob.Dispose();
            return embedder.Forward().Clone();
        }


        private List<Rectangle> DetectFace(UMat image)
        {
            int imgDim = 300;
        ...
        {
            var e=recognizer.Predict(face1);
            Console.SetCursorPosition(0, 7);
            Console.WriteLine($"Distance:{e.Distance} {e.Label}");
            //float[] ar1 = new float[face1.Cols];
            //float[] ar2 = new float[face2.Cols];
            //face1.CopyTo(ar1);
            //face2.CopyTo(ar2);
            //var sum = 0f;
            ...
            //var arr1 = np.array(ar1);
            //var arr2 = np.array(ar1);
            //var ed = arr1 - arr2;
            //ed = np.sum(np.multiply(ed, ed));
            //ed = np.sqrt(ed);
            //var avg = 0.0;
            //for (int i = 0; i < face1.Cols; i++)
            //{
            //    var d = Distance.Euclidean(ar1[i], ar2[i]);
            //    avg += d;
            //}
            //avg = avg / face1.Cols;
            ...
            //var r = svm.Predict(face1.Reshape(1,1));
            //Console.SetCursorPosition(0, 8);
        ...
        private Mat GetPhoto(string path)
        {
            Mat image = new Mat(path);
            var fullFaceRegions = DetectFace(image.GetUMat(Emgu.CV.CvEnum.AccessType.Fast));
            if (fullFaceRegions.Count>0)
            {
